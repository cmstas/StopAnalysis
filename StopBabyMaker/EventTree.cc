...
EventTree::EventTree (const std::string &prefix)
    : prefix_(prefix), kGenweightSize(112)
{
}
...
//      pu_nvtxs = puInfo_nPUvertices().at(6);
      ...
      // Only store the first 112 genweights, unless specified otherwise <-- new after 94X to have > 1000 genweights taking up unnecessary spaces.
      ngenweights = std::min(cms3.genweights().size(), kGenweightSize);
      genweights = vector<float>(cms3.genweights().begin(), cms3.genweights().begin()+ngenweights);
      genweightsID = vector<string>(cms3.genweightsID().begin(), cms3.genweightsID().begin()+std::min(cms3.genweightsID().size(), (unsigned long)ngenweights));
...
*/
    ...
    pfmet_uncorr_jup     = -9999.;
    pfmet_uncorr_phi_jup = -9999.;
    pfmet_uncorr_jdown   = -9999.;
    pfmet_uncorr_phi_jdown = -9999.;
    pfmet_original       = -9999.;
    pfmet_original_phi   = -9999.;
    pfmet_resup          = -9999.;
    pfmet_phi_resup      = -9999.;
    pfmet_resdown        = -9999.;
    pfmet_phi_resdown    = -9999.;
    calomet              = -9999.;
    calomet_phi          = -9999.;
    scale1fb             = -9999.;
    xsec                 = -9999.;
...
void EventTree::SetBranches (TTree* tree)                                                                                          
{
    ...
    tree->Branch("MT2_ll_jup", &MT2_ll_jup);
    ...
    tree->Branch("MT2_ll_jdown", &MT2_ll_jdown);
...
void EventTree::SetPhotonBranches (TTree* tree)
{
    ...
    tree->Branch("ph_MT2_ll",            &ph_MT2_ll);
...
void EventTree::SetZllBranches (TTree* tree)
{
    ...
    tree->Branch("Zll_MT2_ll", &Zll_MT2_ll);
