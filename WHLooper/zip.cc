...
void zip(TString slimFilename, TString whFilename){
	...
	for(int i=0;i < t_wh->GetListOfBranches()->GetEntries();i++){
		...
		else if (this_branch_name.BeginsWith("n") || this_branch_name.EqualTo("year")) this_type=0;
		//Goddamnit- hack for now..
		else if(this_branch_name.EqualTo("stitch") || this_branch_name.EqualTo("pass") || this_branch_name.EqualTo("pass_old") || this_branch_name.EqualTo("duplicate") || this_branch_name.EqualTo("goodrun")) this_type=4;
		else this_type = 1;
