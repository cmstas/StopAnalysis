...
string outputdir = "datacards12fbinv/";
...
bool loadAll(TString signaltype="T2tt", bool fakedata=false);
...
bool reloadSignal(TString signaltype);
//loading signal, background/data files and histograms - split into 2 functions as signal might be reloaded
bool loadDataBGFiles(bool fakedata=false);
bool loadSignalFile(TString signaltype="T2tt");
bool loadDataBGHistos(bool fakedata=false);
bool loadSignalHistos();
...
void makeDataCardsAllScansAllAnalyses(bool dropsigcont, bool correlated, bool fakedata, bool nosigsyst, bool nobgsyst){
  ...
  for(unsigned int i = 0; i<12;++i){
    ...
    bool loaded = false;
    if(i==0) loaded = loadAll(signaltype, fakedata);
    else     loaded = reloadSignal(signaltype);
    if(loaded){
...
void makeDataCardsOneScanAllAnalyses(TString signaltype, bool dropsigcont, bool correlated, bool fakedata, bool nosigsyst, bool nobgsyst){
  ...
  int minLSPMass  = minlspmass(signaltype);
  int maxLSPMass  = maxlspmass(signaltype);
  bool loaded = loadAll(signaltype, fakedata);
  if(loaded){
    for(int analysis = 0; analysis<nanalyses; ++analysis){
      int anmaxbin = analysismaxbin(analysis);
      if(anmaxbin<1) return;
...
void makeDataCardsOneBinBasis(int bin, TString signaltype, int analysis, bool dropsigcont, bool correlated, int stop, int lsp, int charg, float xval, bool fakedata, bool nosigsyst, bool nobgsyst){
  ...
  */
  ...
  data = histData["Observed_yield"]->GetBinContent(b);
  ...
  ++nbgs;
  if(bg1l>0) { bg1lerr = 1.+(TMath::Max(bg1l-hist1l["Data_Up"]->GetBinContent(bW),hist1l["Data_Down"]->GetBinContent(bW)-bg1l)/bg1l); }
  bg1ltop = hist1ltop["ee1lepFromT_yield"]->GetBinContent(bW);
  ++nbgs;
  if(bg1ltop>0) { bg1ltoperr = 2.; } //- no statistical uncertainty on 1ltop !!!
  bgznunu = histZnunu["Yield"]->GetBinContent(bTTZ);
  ++nbgs;
  if(bgznunu>0) { bgznunuerr = 1.+((histZnunu["Stat"]->GetBinContent(bTTZ))); }
  ...
  if(!nobgsyst){
    ...
    getBGUncertainty(v2l["CR2l_jesUp"],        v2l["CR2l_jesDn"],        bg2l, hist2l["CR2l_jesUp"],        hist2l["CR2l_jesDn"],        bLL);
    ...
    getBGUncertainty(v1l["MC_Up"],      v1l["MC_Down"],      bg1l, hist1l["MC_Up"],      hist1l["MC_Down"],      bW);
    if(bW>7&&bW<11) { v1l["MC_Up"] = 2; v1l["MC_Down"] = -1; }
    if(bW<=7||bW>=11){
      ...
      getBGUncertainty(v1l["PDF_Up"],     v1l["PDF_Down"],     bg1l, hist1l["PDF_Up"],     hist1l["PDF_Down"],     bW);
      getBGUncertainty(v1l["Q2_Up"],      v1l["Q2_Down"],      bg1l, hist1l["Q2_Up"],      hist1l["Q2_Down"],      bW);
      getBGUncertainty(v1l["JES_Up"],     v1l["JES_Down"],     bg1l, hist1l["JES_Up"],     hist1l["JES_Down"],     bW);
      getBGUncertainty(v1l["CRCont_Up"],  v1l["CRCont_Down"],  bg1l, hist1l["CRCont_Up"],  hist1l["CRCont_Down"],  bW);
    ...
    getBGErrorRel(vZnunu["Scale"], bgznunu, histZnunu["Scale"], bTTZ);
    getBGErrorRel(vZnunu["PDF"],   bgznunu, histZnunu["PDF"],   bTTZ);
    getBGErrorRel(vZnunu["BTagL"], bgznunu, histZnunu["BTagL"], bTTZ);
    getBGErrorRel(vZnunu["BTagH"], bgznunu, histZnunu["BTagH"], bTTZ);
    getBGErrorRel(vZnunu["Top"],   bgznunu, histZnunu["Top"],   bTTZ);
    getBGErrorRel(vZnunu["PU"],    bgznunu, histZnunu["PU"],    bTTZ);
    getBGErrorRel(vZnunu["JES"],   bgznunu, histZnunu["JES"],   bTTZ);
    getBGErrorRel(vZnunu["Lep"],   bgznunu, histZnunu["Lep"],   bTTZ);
  }//if(!nobgsyst)
  ...
  *fLogStream << "rate        " << sig*lc << "  " << bg2l << "  " << bg1l << "  " << bg1ltop*lc << "  " << bgznunu*lc << endl;// - DELETE *lc XXX
  *fLogStream << "------------" << endl;
  ...
  if(!nobgsyst && bgznunu>0) numnuis += addOneUnc(fLogStream, "BgZnunuStat", bgznunuerr, -1, 4, b,     "lnN");
  ...
  dnerr[0] = vSig["triggerrDn"]; dnerr[3] = vSig["triggerrDn"]; dnerr[4] = vSig["triggerrDn"]; dnerr[1] = v2l["CR2l_cr2lTriggerSFUp"];//obviously need to swap CR2l eff
  uperr[0] = vSig["triggerrUp"]; uperr[3] = vSig["triggerrUp"]; uperr[4] = vSig["triggerrUp"]; uperr[1] = v2l["CR2l_cr2lTriggerSFDn"];
  numnuis += addCorrelatedUnc(fLogStream, "TrigSyst", dnerr, uperr, -1, "lnN");
  dnerr[0] = vSig["lumierr"]; dnerr[3] = vSig["lumierr"]; dnerr[4] = vSig["lumierr"];
  ...
  if(!nobgsyst){
    //correlated
    if(correlated){
      dnerr[1] = v2l["CR2l_bTagEffHFDn"]; dnerr[2] = v1l["btag_HF_Down"]; dnerr[4] = vZnunu["BTagH"];
      uperr[1] = v2l["CR2l_bTagEffHFUp"];   uperr[2] = v1l["btag_HF_Up"];
      numnuis += addCorrelatedUnc(fLogStream, "BHFSystBg", dnerr, uperr, -1, "lnN");
      dnerr[1] = v2l["CR2l_bTagEffLFDn"]; dnerr[2] = v1l["btag_LF_Down"]; dnerr[4] = vZnunu["BTagL"];
      uperr[1] = v2l["CR2l_bTagEffLFUp"];   uperr[2] = v1l["btag_LF_Up"];
      numnuis += addCorrelatedUnc(fLogStream, "BLFSystBg", dnerr, uperr, -1, "lnN");
      dnerr[1] = v2l["CR2l_lepSFDn"]; dnerr[4] = vZnunu["Lep"];
      uperr[1] = v2l["CR2l_lepSFUp"];   
      numnuis += addCorrelatedUnc(fLogStream, "LEffSystBg", dnerr, uperr, -1, "lnN");
      /*dnerr[1] = v2l["CR2l_PDFdown"];*/ dnerr[2] = v1l["PDF_Down"]; dnerr[4] = vZnunu["PDF"];
      /*uperr[1] = v2l["CR2l_PDFup"];*/   uperr[2] = v1l["PDF_Up"];
      numnuis += addCorrelatedUnc(fLogStream, "PDFSystBg", dnerr, uperr, -1, "lnN");
      dnerr[1] = v2l["CR2l_q2Dn"]; dnerr[2] = v1l["Q2_Down"]; dnerr[4] = vZnunu["Scale"];
      uperr[1] = v2l["CR2l_q2Up"];  uperr[2] = v1l["Q2_Up"];
      numnuis += addCorrelatedUnc(fLogStream, "QsqSystBg", dnerr, uperr, -1, "lnN");
      dnerr[1] = v2l["CR2l_jesDn"]; dnerr[2] = v1l["JES_Down"]; dnerr[4] = vZnunu["JES"];
      uperr[1] = v2l["CR2l_jesUp"];   uperr[2] = v1l["JES_Up"];
      numnuis += addCorrelatedUnc(fLogStream, "JESSystBg", dnerr, uperr, -1, "lnN");
      ...
      numnuis += addOneUnc(fLogStream, "JESSyst2l",  v2l["CR2l_jesDn"],       v2l["CR2l_jesUp"],       1, -1, "lnN");
      ...
      numnuis += addOneUnc(fLogStream, "PDFSyst1l",  v1l["PDF_Down"],     v1l["PDF_Up"],     2, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "muRFSyst1l", v1l["Q2_Down"],      v1l["Q2_Up"],      2, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "JESSysy1l",  v1l["JES_Down"],     v1l["JES_Up"],     2, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "METSyst1l",  v1l["metres_Down"],  v1l["metres_Up"],  2, -1, "lnN");
      //bgznunu
      numnuis += addOneUnc(fLogStream, "BHFSFSystZ",  vZnunu["BTagL"],  -1, 4, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "BLFSFSystZ",  vZnunu["BTagH"],  -1, 4, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "LEffSystZ", vZnunu["Lep"],       -1, 4, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "JESSystZ",    vZnunu["JES"],   -1, 4, -1, "lnN");
      numnuis += addOneUnc(fLogStream, "PDFSystZ",    vZnunu["PDF"],   -1, 4,  b, "lnN");
      numnuis += addOneUnc(fLogStream, "QsqSystZ",    vZnunu["Scale"], -1, 4,  b, "lnN");
    }
    //bg2l
    ...
    numnuis += addOneUnc(fLogStream, "MCSyst2l",  v2l["CR2l_mcStatsUp"],    v2l["CR2l_mcStatsDn"],    1, -1, "lnN");
    //bg1l
    
    /*if(b<=7||b>=11)*/ numnuis += addOneUnc(fLogStream, "MCSyst1l",     v1l["MC_Down"],      v1l["MC_Up"],      2,  b, "lnN");
    //else numnuis += addOneUnc(fLogStream, "MCSyst1l",     v1l["MC_Down"],      v1l["MC_Up"],      2,  b, "lnN");
    ////numnuis += addOneUnc(fLogStream, "WbxsSyst1l",   v1l["Wb_Down"],      v1l["Wb_Up"],      2, -1, "lnN");
    if(bW<=7||bW>=11) numnuis += addOneUnc(fLogStream, "WbxsSyst1l",   v1l["Wb_Up"],      -1,      2, -1, "lnN");
    //numnuis += addOneUnc(fLogStream, "WwidthSyst1l", v1l["W_width_Down"], v1l["W_width_Up"], 2, -1, "lnN");
    numnuis += addOneUnc(fLogStream, "ContSyst1l",   v1l["CRCont_Down"],  v1l["CRCont_Up"],  2, -1, "lnN");
    //numnuis += addOneUnc(fLogStream, "NuPtSyst1l",   v1l["nu_pT_Down"],   v1l["nu_pT_Up"],   2, -1, "lnN");
    numnuis += addOneUnc(fLogStream, "METSyst1l",  v1l["metres_Down"],  v1l["metres_Up"],  2, -1, "lnN");

    //bgznunu
    numnuis += addOneUnc(fLogStream, "TPtSystZ", vZnunu["Top"], -1, 4,  b, "lnN");
    numnuis += addOneUnc(fLogStream, "PUSystZ",  vZnunu["PU"],  -1, 4, -1, "lnN");
    //numnuis += addOneUnc(fLogStream, "PDFSystZ", vZnunu["PDF"], -1, 4, -1, "lnN");//CHECK THAT THIS IS NOT DOUBLE COUNTED
  }
  *fLogStream << endl;
...
bool loadDataBGFiles(bool fakedata){
  ...
  if(!f2l->IsZombie()&&f2l->IsOpen()){
    ...
    filename = inputdir + "h_limit__lostLep.root";
  ...
  if(f1l->IsOpen()){
    ...
    filename = inputdir + "WJets12fbinv.root";
  ...
  if(f1ltop->IsOpen()){
    ...
    filename = inputdir + "h_limit__1lepFromT.root";
  ...
  if(fznunu->IsOpen()){
    ...
    filename = inputdir + "Znunu_wRelErrors.root";
  ...
  if(fdata->IsOpen()){
    ...
    if(!fakedata) {
      filename= inputdir + "h_limit__data.root";
...
//clear - returns false if loading of at least one file failed (i.e. file non existing)
bool loadSignalFile(TString signaltype){
  fsig = new TFile();
  TString filename;
  if(signaltype==lastsignalname){//if already open, don't reopen
    if(fsig->IsOpen()){
      ...
      filename = inputdir + "Histos_Signal_" + signaltype + "_all.root";
...
bool loadDataBGHistos(bool fakedata){
  ...
  histData["Observed_yield"];
  if(fakedata) { histData["Observed_yield"] = (TH1D*)hist2l["CR2l_yield"]->Clone("Observed_yield"); }
  else {
...
bool loadAll(TString signaltype, bool fakedata){
  
  bool l1 = loadDataBGFiles(fakedata);
  bool l2 = loadSignalFile(signaltype);
  bool l3 = loadDataBGHistos(fakedata);
  bool l4 = loadSignalHistos();
  return (l1&&l2&&l3&&l4);
}
...
int checkbin(int analysis, int bin, double stop, double lsp, bool dropsigcont){
  ...
  if(bW>hist1ltop["ee1lepFromT_yield"]->GetNbinsX()) return -1;
  if(bTTZ>histZnunu["Yield"]->GetNbinsX()) return -1;
  if(b>histData["Observed_yield"]->GetNbinsX()) return -1;
  ...
  if(hist1ltop["ee1lepFromT_yield"]->GetBinContent(bW)<0) return -1;
  if(histZnunu["Yield"]->GetBinContent(bTTZ)<0) return -1;
  if(histData["Observed_yield"]->GetBinContent(b)<0) return -1;
